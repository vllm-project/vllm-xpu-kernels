...
macro(append_cmake_prefix_path PKG EXPR)
  run_python(_PREFIX_PATH "import ${PKG}; print(${EXPR})"
...
function(hipify_sources_target OUT_SRCS NAME ORIG_SRCS)
  ...
  #
  # Generate ROCm/HIP source file names from CUDA file names. Since HIP files
  # are generated code, they will appear in the build area
  ...
  add_custom_target(
    hipify${NAME}
    COMMAND ${CMAKE_SOURCE_DIR}/cmake/hipify.py -p ${CMAKE_SOURCE_DIR}/csrc -o
            ${CSRC_BUILD_DIR} ${SRCS}
...
function(get_torch_gpu_compiler_flags OUT_GPU_FLAGS GPU_LANG)
  if(${GPU_LANG} STREQUAL "CUDA")
    ...
    if(CUDA_VERSION VERSION_GREATER_EQUAL 12.0)
      list(REMOVE_ITEM GPU_FLAGS "-D__CUDA_NO_HALF_OPERATORS__"
...
# `CUDA_ARCH_FLAGS`.
#
# Example: CMAKE_CUDA_FLAGS="-Wall -gencode arch=compute_70,code=sm_70 -gencode
# arch=compute_75,code=sm_75" clear_cuda_arches(CUDA_ARCH_FLAGS)
# CUDA_ARCH_FLAGS="-gencode arch=compute_70,code=sm_70;-gencode
# arch=compute_75,code=sm_75" CMAKE_CUDA_FLAGS="-Wall"
#
macro(clear_cuda_arches CUDA_ARCH_FLAGS)
  ...
  # Remove all `-gencode` flags from `CMAKE_CUDA_FLAGS` since they will be
  # modified and passed back via the `CUDA_ARCHITECTURES` property.
...
#
# Example: CUDA_ARCH_FLAGS="-gencode arch=compute_75,code=sm_75;...;-gencode
# arch=compute_90a,code=sm_90a" extract_unique_cuda_archs_ascending(OUT_ARCHES
# CUDA_ARCH_FLAGS) OUT_ARCHES="7.5;...;9.0"
...
#
# Example: set_gencode_flag_for_srcs( SRCS "foo.cu" ARCH "compute_75" CODE
# "sm_75") adds: "-gencode arch=compute_75,code=sm_75" to the compile options
# for `foo.cu` (only for the CUDA language).
#
macro(set_gencode_flag_for_srcs)
  ...
  set_property(
    SOURCE ${arg_SRCS}
    APPEND
    PROPERTY COMPILE_OPTIONS "$<$<COMPILE_LANGUAGE:CUDA>:${_FLAG}>")
...
#
# arguments are: SRCS: list of source files CUDA_ARCHS: list of CUDA
# architectures in the form `<major>.<minor>[letter]` BUILD_PTX_FOR_ARCH: if set
# to true, then the PTX code will be built for architecture `BUILD_PTX_FOR_ARCH`
# if there is a CUDA_ARCH in CUDA_ARCHS that is larger than BUILD_PTX_FOR_ARCH.
#
macro(set_gencode_flags_for_srcs)
  ...
  foreach(_ARCH ${arg_CUDA_ARCHS})
    ...
    if(NOT _HAS_PTX EQUAL -1)
      ...
      set_gencode_flag_for_srcs(
        SRCS ${arg_SRCS} ARCH "compute_${_STRIPPED_ARCH}" CODE
        "sm_${_STRIPPED_ARCH}")
      set_gencode_flag_for_srcs(
        SRCS ${arg_SRCS} ARCH "compute_${_STRIPPED_ARCH}" CODE
        "compute_${_STRIPPED_ARCH}")
    else()
  ...
  if(${arg_BUILD_PTX_FOR_ARCH})
    ...
    if(_HIGHEST_ARCH VERSION_GREATER_EQUAL ${arg_BUILD_PTX_FOR_ARCH})
      ...
      set_gencode_flag_for_srcs(SRCS ${arg_SRCS} ARCH "compute_${_PTX_ARCH}"
...
# architecture in `SRC_CUDA_ARCHS`. The loose intersection is defined as: { max{
# x \in tgt | x <= y } | y \in src, { x \in tgt | x <= y } != {} } where `<=` is
# the version comparison operator. In other words, for each version in
# `TGT_CUDA_ARCHS` find the highest version in `SRC_CUDA_ARCHS` that is less or
# equal to the version in `TGT_CUDA_ARCHS`. We have special handling for x.0a,
# if x.0a is in `SRC_CUDA_ARCHS` and x.0 is in `TGT_CUDA_ARCHS` then we should
# remove x.0a from `SRC_CUDA_ARCHS` and add x.0a to the result (and remove x.0
# from TGT_CUDA_ARCHS). The result is stored in `OUT_CUDA_ARCHS`.
#
# Example: SRC_CUDA_ARCHS="7.5;8.0;8.6;9.0;9.0a" TGT_CUDA_ARCHS="8.0;8.9;9.0"
# cuda_archs_loose_intersection(OUT_CUDA_ARCHS SRC_CUDA_ARCHS TGT_CUDA_ARCHS)
# OUT_CUDA_ARCHS="8.0;8.6;9.0;9.0a"
#
# Example With PTX: SRC_CUDA_ARCHS="8.0+PTX" TGT_CUDA_ARCHS="9.0"
# cuda_archs_loose_intersection(OUT_CUDA_ARCHS SRC_CUDA_ARCHS TGT_CUDA_ARCHS)
# OUT_CUDA_ARCHS="8.0+PTX"
#
function(cuda_archs_loose_intersection OUT_CUDA_ARCHS SRC_CUDA_ARCHS
         TGT_CUDA_ARCHS)
  ...
  # if x.0a is in SRC_CUDA_ARCHS and x.0 is in CUDA_ARCHS then we should remove
  # x.0a from SRC_CUDA_ARCHS and add x.0a to _CUDA_ARCHS
  set(_CUDA_ARCHS)
  ...
  foreach(_ARCH ${_TGT_CUDA_ARCHS})
    ...
    foreach(_SRC_ARCH ${_SRC_CUDA_ARCHS})
      ...
      string(REGEX REPLACE "^([0-9]+)\\..*$" "\\1" SRC_ARCH_MAJOR
                           "${_SRC_ARCH}")
      # Check version-less-or-equal, and allow PTX arches to match across majors
      if(_SRC_ARCH VERSION_LESS_EQUAL _ARCH)
        if(_SRC_ARCH IN_LIST _PTX_ARCHS OR SRC_ARCH_MAJOR STREQUAL
                                           TGT_ARCH_MAJOR)
...
# `GPU_SUPPORTED_ARCHES`. Sets the final set of architectures in `GPU_ARCHES`.
# This only applies to the HIP language since for CUDA we set the architectures
# on a per file basis.
#
...
macro(override_gpu_arches GPU_ARCHES GPU_LANG GPU_SUPPORTED_ARCHES)
  ...
  if(${GPU_LANG} STREQUAL "HIP")
    ...
    #
    # If PYTORCH_ROCM_ARCH env variable exists, then we take it as a list, if
    # not, then we use CMAKE_HIP_ARCHITECTURES which was generated by calling
    # "rocm_agent_enumerator" in "enable_language(HIP)" (in file
    ...
    #
    # Find the intersection of the supported + detected architectures to set the
    # module architecture flags.
    #
    set(${GPU_ARCHES})
...
#
# Define a target named `GPU_MOD_NAME` for a single extension. The arguments
# are:
#
# DESTINATION <dest>         - Module destination directory. LANGUAGE <lang> -
# The GPU language for this module, e.g CUDA, HIP, etc. SOURCES <sources> - List
# of source files relative to CMakeLists.txt directory.
#
# Optional arguments:
#
# ARCHITECTURES <arches>     - A list of target GPU architectures in cmake
# format. Refer `CMAKE_CUDA_ARCHITECTURES` documentation and
# `CMAKE_HIP_ARCHITECTURES` for more info. ARCHITECTURES will use cmake's
# defaults if not provided. COMPILE_FLAGS <flags>      - Extra compiler flags
# passed to NVCC/hip. INCLUDE_DIRECTORIES <dirs> - Extra include directories.
# LIBRARIES <libraries>      - Extra link libraries. LINK_FLAGS <flags> - Extra
# link flags. WITH_SOABI                 - Generate library with python SOABI
# suffix name. USE_SABI <version>         - Use python stable api <version>
#
...
function(define_gpu_extension_target GPU_MOD_NAME)
  ...
  if(GPU_ARCHITECTURES)
    set_target_properties(
      ${GPU_MOD_NAME} PROPERTIES ${GPU_LANGUAGE}_ARCHITECTURES
                                 "${GPU_ARCHITECTURES}")
  endif()
  ...
  target_compile_options(
    ${GPU_MOD_NAME}
    PRIVATE $<$<COMPILE_LANGUAGE:${GPU_LANGUAGE}>:${GPU_COMPILE_FLAGS}>)
...
function(add_xe2_kernel_library LIBRARY_NAME)
  cmake_parse_arguments(
    ...
    "EXCLUDE_SOURCES" # Multi-value keywords
  )
  ...
  if(ARG_EXCLUDE_SOURCES)
    list(REMOVE_ITEM KERNEL_SOURCES ${ARG_EXCLUDE_SOURCES})
  endif()
...
#
# Arguments: LIBRARY_NAME: Name of the library to create (e.g.,
# grouped_gemm_xe_default) DESTINATION: Installation destination directory
# (optional, defaults to vllm_xpu_kernels) INCLUDE_CMAKE_SOURCE_DIR: Optional
# flag to include ${CMAKE_SOURCE_DIR} in include directories
#
