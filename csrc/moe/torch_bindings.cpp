...
TORCH_LIBRARY_EXPAND(TORCH_EXTENSION_NAME, m) {
  ...
  m.def(
      ...
      "Tensor unpermuted_row_to_permuted_row, Tensor expert_first_token_offset, int num_experts) -> ()");
  m.impl("moe_gather", torch::kXPU, &moe_gather);
  m.def(
      "fused_moe_prologue(Tensor input, Tensor token_selected_experts, "
      "Tensor "
      "token_final_scales, Tensor workspace, int hidden_size, int inter_size, int ep_rank, int ep_size,"
