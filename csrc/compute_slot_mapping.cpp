...
template <int TOTAL_CP_WORLD_SIZE,
          int TOTAL_CP_RANK,
          int CP_KV_CACHE_INTERLEAVE_SIZE,
          int64_t PAD_ID>
class ComputeSlotMappingKernel {
 public:
  ComputeSlotMappingKernel(int64_t num_tokens,
  ...
  void operator()(sycl::nd_item<1> item) const {
    ...
    if (req_idx == num_groups - 1) {
      for (int64_t base = num_tokens_; base < max_num_tokens_; base += BLOCK_SIZE) {
    ...
    for (int64_t base = start_idx; base < end_idx; base += BLOCK_SIZE) {
      ...
      const int64_t block_number = static_cast<int64_t>(
          block_table_[row_offset + block_index]);
      ...
      const int64_t slot_id =
          block_number * block_size_ + local_block_offset;
...
#define VLLM_DISPATCH_CP_WORLD_SIZE(WS, CONST_WS, ...)                 \
  switch (WS) {                                                        \
    case 1: { constexpr int CONST_WS = 1; __VA_ARGS__(); break; }      \
    case 2: { constexpr int CONST_WS = 2; __VA_ARGS__(); break; }      \
    case 4: { constexpr int CONST_WS = 4; __VA_ARGS__(); break; }      \
    case 8: { constexpr int CONST_WS = 8; __VA_ARGS__(); break; }      \
    default:                                                           \
...
#define VLLM_DISPATCH_CP_RANK(RANK, CONST_WS, CONST_RANK, ...)             \
  switch (RANK) {                                                          \
    case 0: { constexpr int CONST_RANK = 0; __VA_ARGS__(); break; }        \
    case 1: { if constexpr (CONST_WS > 1) {                                \
                constexpr int CONST_RANK = 1; __VA_ARGS__(); } break; }    \
    case 2: { if constexpr (CONST_WS > 2) {                                \
                constexpr int CONST_RANK = 2; __VA_ARGS__(); } break; }    \
    case 3: { if constexpr (CONST_WS > 3) {                                \
                constexpr int CONST_RANK = 3; __VA_ARGS__(); } break; }    \
    case 4: { if constexpr (CONST_WS > 4) {                                \
                constexpr int CONST_RANK = 4; __VA_ARGS__(); } break; }    \
    case 5: { if constexpr (CONST_WS > 5) {                                \
                constexpr int CONST_RANK = 5; __VA_ARGS__(); } break; }    \
    case 6: { if constexpr (CONST_WS > 6) {                                \
                constexpr int CONST_RANK = 6; __VA_ARGS__(); } break; }    \
    case 7: { if constexpr (CONST_WS > 7) {                                \
                constexpr int CONST_RANK = 7; __VA_ARGS__(); } break; }    \
    default:                                                               \
      TORCH_CHECK(false, "Unsupported cp_rank: ", RANK,                    \
                  " for world_size=", CONST_WS);                           \
  }

#define VLLM_DISPATCH_CP_INTERLEAVE(IL, CONST_IL, ...)                 \
  switch (IL) {                                                        \
    case 1:  { constexpr int CONST_IL = 1;  __VA_ARGS__(); break; }    \
    case 16: { constexpr int CONST_IL = 16; __VA_ARGS__(); break; }    \
    case 64: { constexpr int CONST_IL = 64; __VA_ARGS__(); break; }    \
    default:                                                           \
...
void compute_slot_mapping(int64_t num_reqs,
  ...
  TORCH_CHECK(block_table.size(0) * block_table.size(1) >=
  ...
  TORCH_CHECK(pad_id == -1, "Only PAD_SLOT_ID=-1 is templated; "
              "got pad_id=", pad_id);
  ...
  VLLM_DISPATCH_CP_WORLD_SIZE(total_cp_world_size, CP_WS, [&] {
    VLLM_DISPATCH_CP_RANK(total_cp_rank, CP_WS, CP_RANK, [&] {
      VLLM_DISPATCH_CP_INTERLEAVE(cp_kv_cache_interleave_size, CP_IL, [&] {
        using Kernel = vllm::ComputeSlotMappingKernel<
            CP_WS, CP_RANK, CP_IL, /*PAD_ID=*/-1>;

        const sycl::range<1> global((num_reqs + 1) *
                                    static_cast<size_t>(Kernel::BLOCK_SIZE));
        const sycl::range<1> local(Kernel::BLOCK_SIZE);

        Kernel k(num_tokens, max_num_tokens, qsl_ptr, pos_ptr,
                 bt_ptr, bt_stride, bs, sm_ptr);
