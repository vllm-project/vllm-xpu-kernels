...
// For async H2D copy, it will check the src is allocated by SYCL API or
...
void xpuMemcpy(void* dst, const void* src, size_t n_bytes, xpuMemcpyKind kind) {
  switch (kind) {
    case HostToDevice:
      ...
      memcpyHostToDevice(dst, src, n_bytes, false, nullptr, false);
      break;
    case DeviceToHost:
      // for synchronous copy, the context of host data pointer is unnecessary.
      memcpyDeviceToHost(dst, src, n_bytes, false, nullptr, false);
      break;
    case DeviceToDevice:
