...
static inline void dnnl_matmul_w8a8_fp8(
  ...
  if (mat2.dim() == 3) {
    ...
    const bool weight_bkn = mat2.size(1) == k;
    const bool weight_bnk = mat2.size(2) == k;
    TORCH_CHECK(
        weight_bkn || weight_bnk,
        "Batched fp8 matmul expects weight shape [B, K, N] or [B, N, K], got ",
        mat2.sizes(),
        ".");
    ...
    memory::dims src_strides = get_onednn_strides(mat1);
    memory::dims wei_dims = {batch, k, n};
    memory::dims wei_strides = get_onednn_strides(mat2);
    memory::dims dst_dims = {batch, m, n};
    memory::dims dst_strides = get_onednn_strides(result);

    auto& matmul_ext = matmul_primitive_create_and_cache(
        jd,
        is_nt ? trans_type_t::nt : trans_type_t::nn,
        b_type,
        src_dims,
        wei_dims,
        dst_dims,
