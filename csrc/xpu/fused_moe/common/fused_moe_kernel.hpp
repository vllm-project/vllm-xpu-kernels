...
CUTE_DEVICE void FusedMOEUp(
  ...
  for (int i = 0; i < num_experts; ++i) {
    ...
    ElementB* ptr_B1_curr_batch = ptr_B_curr_batch;
    ElementB* ptr_B2_curr_batch = nullptr;
    if constexpr (is_B_4bits) {
      ptr_B2_curr_batch = ptr_B1_curr_batch + gemm_n * gemm_k / 2;
    } else {
      if constexpr (actual_layout_of_B == 'C') {
        ptr_B2_curr_batch = ptr_B1_curr_batch + gemm_n;
      } else {
        ptr_B2_curr_batch = ptr_B1_curr_batch + gemm_n * gemm_k;
      }
    }

    auto A_tensor = make_moe_tensor<ElementA, LayoutKindA>(
        ptr_A_curr_batch, gemm_m, gemm_k);
    auto make_B_tensor = [&](ElementB* ptr_B_curr_batch) {
      auto shape = make_shape(gemm_n, gemm_k);
      if constexpr (is_B_int4) {
        return make_tensor(
            make_gmem_ptr(reinterpret_cast<int4_t*>(ptr_B_curr_batch)),
            make_layout(shape, make_stride(gemm_k, _1{})));
      } else if constexpr (is_B_mxfp4) {
        return make_tensor(
            make_gmem_ptr(reinterpret_cast<float_e2m1_t*>(ptr_B_curr_batch)),
            make_layout(shape, make_stride(gemm_k, _1{})));
      } else {
        if constexpr (actual_layout_of_B == 'C') {
          return make_tensor(
              make_gmem_ptr(ptr_B_curr_batch),
              make_layout(shape, make_stride(_1{}, 2 * gemm_n)));
        } else {
          return make_tensor(
              make_gmem_ptr(ptr_B_curr_batch),
              make_layout(shape, make_stride(gemm_k, _1{})));
