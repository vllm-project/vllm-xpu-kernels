...
// This is generated by CMake based on VLLM_PAGED_DECODE_CONFIG.
// In "full" mode all policies are enabled; in selective mode only the
// configured subset is enabled, and unconfigured paths emit a runtime error.
...
void decode_policy_dispatch_func(
  ...
  if constexpr (!is_decode_policy_enabled<decode_policy>::value) {
    ...
    decode_policy_dispatch_impl<decode_policy>(queue, cuQKType, args);
  }
}
