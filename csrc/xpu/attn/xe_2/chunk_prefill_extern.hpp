...
// =============================================================================
// Prevent implicit instantiation of policy_dispatch_typed_impl in translation
// units that include chunk_prefill.hpp. Each specialization is explicitly
// instantiated in its own .cpp file generated by CMake.

// Helper macro to declare a single extern template instantiation
#define DECLARE_TYPED_DISPATCH_EXTERN(                  \
    POLICY, PAGED, CAUSAL, LOCAL, SINK, EQ, EK, EV, EO) \
  extern template void policy_dispatch_typed_impl<      \
      ...
      EK,                                               \
      EV,                                               \
      EO>(sycl::queue & queue, const chunk_prefill_args_t& args);

// Generate all dtype combinations for a given (policy, bool) combo
#define DECLARE_ALL_DTYPES(POLICY, PAGED, CAUSAL, LOCAL, SINK)            \
  DECLARE_TYPED_DISPATCH_EXTERN(                                          \
      POLICY, PAGED, CAUSAL, LOCAL, SINK, half_t, half_t, half_t, half_t) \
  DECLARE_TYPED_DISPATCH_EXTERN(                                          \
      POLICY,                                                             \
      PAGED,                                                              \
      CAUSAL,                                                             \
...
#define DECLARE_FOR_SINK(POLICY, PAGED, CAUSAL, LOCAL)    \
  DECLARE_ALL_DTYPES(POLICY, PAGED, CAUSAL, LOCAL, false) \
  DECLARE_ALL_DTYPES(POLICY, PAGED, CAUSAL, LOCAL, true)
...
#undef DECLARE_ALL_DTYPES
#undef DECLARE_TYPED_DISPATCH_EXTERN
