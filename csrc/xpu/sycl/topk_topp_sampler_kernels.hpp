...
struct top_k_only_kernel {
 public:
  ...
  operator()(sycl::nd_item<1> item) const {
    ...
    float* processed_logprobs_ptr = processed_logprobs + batch_id * vocal_size + local_offset;
    float* logits_ptr = logits + batch_id * vocal_size + local_offset;
  ...
  operator()(sycl::nd_item<1> item) const {
    ...
    if(top_p_value != 1.0f){
        do{
            ...
            double pivot_count = sycl::reduce_over_group(group, pivot_count_local, sycl::plus<>());
...
void topk_topp_sampler_kernel_launcher(
    ...
    if(top_k != nullptr && top_p == nullptr){
        ...
        std::cout << "Launch top_p_only_kernel" << std::endl;
        using KERNEL_TOPP_ONLY = top_p_only_kernel;
