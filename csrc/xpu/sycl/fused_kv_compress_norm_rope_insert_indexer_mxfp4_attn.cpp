...
template <int N>
using uvec = unsigned int __attribute__((ext_vector_type(N)));
...
#else
  for (int k = 0; k < 4; ++k)
    out[k] = base[k];  // host fallback (unused)
#endif
}
...
inline void block2d_load_kv_score_sg32(
    const bf16* base,
    int64_t state_width,
    int64_t pos_stride,
    int n_pos,
  ...
  bool contiguous =
      (state_width == HEAD_SIZE) && (n_pos == 1 || pos_stride == 2 * HEAD_SIZE);
  ...
  #pragma unroll
    ...
    for (int p = 0; p < n_pos; ++p) {
      ...
      block2d_load_sg32(
          base + p * pos_stride + state_width, out + p * 2 * elems + elems);
    }
  }
#endif
}
...
// one subgroup (SG_SIZE lanes), so the work-group size is TOKENS_PER_WG *
// SG_SIZE. Packing several tokens per work-group improves EU occupancy and
// amortizes launch overhead. No group barriers are used, so subgroups (tokens)
// run and early-exit fully independently. Overridable via -DTOKENS_PER_WG=N for
// tuning. Tune via -DTOKENS_PER_WG=N for the target device / workload.
...
#pragma unroll
    ...
    for (; r + 1 < N_GATHER; r += 2) {
      ...
      if (valid0) {
        ...
        int64_t row_base0 =
            static_cast<int64_t>(blk_num0) * state_cache_stride0 +
            static_cast<int64_t>(blk_off0) * state_cache_stride1 + hoff0;
      ...
      if (valid1) {
        ...
        int64_t row_base1 =
            static_cast<int64_t>(blk_num1) * state_cache_stride0 +
            static_cast<int64_t>(blk_off1) * state_cache_stride1 + hoff1;
        row_ptr1 = state_cache_ptr + row_base1;
      }

      // Batched path: both valid, same page block, consecutive offsets, same
      // hoff
      ...
      if (can_batch) {
        ...
        block2d_load_kv_score_sg32(
            row_ptr0, state_width, state_cache_stride1, 2, buf2);
        bf16* kv1_buf = buf2;
...
#pragma unroll
    ...
    // (masks 1,2,4,8 stay inside the 16-lane group, bit-4 separates the
    // halves).
    // ============================================================================
...
#pragma unroll
    for (int pp = 0; pp < ELEMENTS_PER_LANE / 2; ++pp) {
      ...
      float a =
          sycl::fmax(sycl::fabs(m_run[2 * pp]), sycl::fabs(m_run[2 * pp + 1]));
#pragma unroll
      ...
      // block id: pair pp, lower half (lanes 0..15) -> 2*pp, upper half ->
      // 2*pp+1
      int block_id = 2 * pp + (lane >> 4);
      // one lane per half writes the scale (lane 0 for lower, lane 16 for
      // upper)
...
void launch_kernel(sycl::queue& q, FusedMxfp4Kernel<CR, OV> kernel) {
  ...
  // multiple of TOKENS_PER_WG (extra subgroups early-exit via the bounds
  // check).
  const size_t groups0 =
