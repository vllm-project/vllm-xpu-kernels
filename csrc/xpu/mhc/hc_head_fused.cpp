...
public:
    ...
    void operator()(const Params& p, char* smem_buf) const {
        ...
        for (int k = tid * VEC_DOT; k < p.hc_h; k += WG_SIZE * VEC_DOT) {
            ...
            #pragma unroll
            for (int j = 0; j < HC; ++j)
                fn_tile[j] = *reinterpret_cast<const vec_f32_dot_t*>(p.fn + j * p.hc_h + k);
            ...
            for (int t = 0; t < BLOCK_M; ++t) {
                ...
                if (tok >= p.N)
                    break;
                auto xv = *reinterpret_cast<const vec_bf16_dot_t*>(p.hs_flat + tok * p.hc_h + k);
                float xf[VEC_DOT];
                #pragma unroll
        ...
        for (int t = 0; t < BLOCK_M; ++t) {
            ...
            float rrms = sycl::native::rsqrt(tok_scratch[0] / static_cast<float>(p.hc_h) + p.rms_eps);
            if (tid < HC) {
                float mix_val = tok_scratch[tid + 1] * rrms;
                pre_s[t * HC + tid] = sigmoid(sycl::fma(mix_val, p.hc_scale[0], p.hc_base[tid])) + p.hc_eps;
            }
        }
        sycl::group_barrier(it.get_group());
        ...
        for (int t = 0; t < BLOCK_M; ++t) {
            ...
            for (int k = tid * VEC_OUT; k < p.H; k += WG_SIZE * VEC_OUT) {
                ...
                #pragma unroll
                for (int m = 0; m < HC; ++m) {
                    auto rv = *reinterpret_cast<const vec_bf16_out_t*>(p.hs_flat + (tok * HC + m) * p.H + k);
                    #pragma unroll
                    for (int v = 0; v < VEC_OUT; ++v)
...
sycl::event launch_hc_head_fused_opt(
    ...
    const int smem_size = static_cast<int>(
        (HcHeadFusedOptKernel::NUM_RED * HcHeadFusedOptKernel::NUM_SG * HcHeadFusedOptKernel::BLOCK_M
         + HC * HcHeadFusedOptKernel::BLOCK_M) * sizeof(float));
    ...
    auto event = launch<cutlass::device_kernel<HcHeadFusedOptKernel>>(
        launch_policy{
            sycl_grid,
            sycl_block,
    ...
    auto event =
        compat::experimental::launch<cutlass::device_kernel<HcHeadFusedOptKernel>, HcHeadFusedOptKernel>(
            policy, q, params);
#endif
