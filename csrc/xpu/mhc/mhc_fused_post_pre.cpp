...
mhc_fused_post_pre(
    ...
    TORCH_CHECK(x.is_xpu() && residual.is_xpu() && post_layer_mix.is_xpu() && comb_res_mix.is_xpu()
                    && fn.is_xpu() && hc_scale.is_xpu() && hc_base.is_xpu(),
    ...
    TORCH_CHECK(residual_c.dim() == 3, "mhc_fused_post_pre: residual must be [N, HC, H]");
    TORCH_CHECK(x_c.dim() == 2, "mhc_fused_post_pre: x must be [N, H]");
    TORCH_CHECK(c_c.dim() == 2, "mhc_fused_post_pre: post_layer_mix must be [N, HC] or [N, HC, 1]");
    TORCH_CHECK(a_c.dim() == 3, "mhc_fused_post_pre: comb_res_mix must be [N, HC, HC]");
    TORCH_CHECK(residual_c.size(0) == x_c.size(0), "mhc_fused_post_pre: batch mismatch");
    TORCH_CHECK(residual_c.size(0) == c_c.size(0), "mhc_fused_post_pre: post_layer_mix batch mismatch");
    TORCH_CHECK(residual_c.size(0) == a_c.size(0), "mhc_fused_post_pre: comb_res_mix batch mismatch");
    TORCH_CHECK(residual_c.size(1) == HC, "mhc_fused_post_pre: residual dim-2 must be HC=4");
    TORCH_CHECK(c_c.size(1) == HC, "mhc_fused_post_pre: post_layer_mix shape mismatch");
    TORCH_CHECK(a_c.size(1) == HC && a_c.size(2) == HC, "mhc_fused_post_pre: comb_res_mix shape mismatch");
    TORCH_CHECK(residual_c.size(2) == x_c.size(1), "mhc_fused_post_pre: hidden size mismatch");

    const int64_t N = residual_c.size(0);
    const int64_t H = residual_c.size(2);
    TORCH_CHECK(H % 8 == 0, "mhc_fused_post_pre: hidden_size must be a multiple of 8 for vectorized loads/stores");
    const int64_t HC2 = HC * HC;
    const int64_t HC3 = HC * 2 + HC2;
    TORCH_CHECK(fn_c.size(0) == HC3 && fn_c.size(1) == HC * H, "mhc_fused_post_pre: fn shape mismatch");
    TORCH_CHECK(hc_scale.numel() == 3, "mhc_fused_post_pre: hc_scale must have 3 elements");
    TORCH_CHECK(hc_base.numel() == HC3, "mhc_fused_post_pre: hc_base must have HC3 elements");
