...
//       - Phase 0: reduce partial_C and partial_sqrsum, apply RMS-norm → mixes in SLM
...
//   - Fused reduce+Stage2 eliminates intermediate rms_mixes global memory traffic
...
//                                                       |
//   residual[M,HC,H] bf16 ---- Fused Reduce+Stage2 --> post_mix, comb_mix, layer_input
...
template <int PrefetchDist,
          class ATensor, class BTensor, class WSTensor,
          class TiledMMA>
void
mhc_pre_splitk_device(ATensor   const& A,          // (M, K) bf16
                      ...
                      int              M)
{
    ...
    CUTE_UNROLL
    for (int i = 0; i < prefetch_dist && k_tile_prefetch < total_k_tiles; i++, k_tile_prefetch++) {
            ...
            [=](sycl::nd_item<1> item) [[sycl::reqd_sub_group_size(SG_SIZE)]]
        {
            ...
            if (sg_id == 0) {
                ...
                if (lane < HC) {
                    ...
                    float post_logits = slm[lane + HC] * hc_scale[1] + hc_base[lane + HC];
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> mhc_pre(
    ...
    TORCH_CHECK(residual.is_xpu() && fn.is_xpu() && hc_scale.is_xpu() && hc_base.is_xpu(), "mhc_pre: tensors must be on XPU");
    ...
    TORCH_CHECK(H % 8 == 0, "mhc_pre: hidden_size must be a multiple of 8 for vectorized loads/stores");
