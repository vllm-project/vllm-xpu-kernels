...
class DensePrefillFwdKernel {
 public:
  ...
  using FragA = expand_sg_fragment_t<
      SingleFragA,
      1,
      Traits::D_V_PER_SPLIT / get<1>(TileShapePV{})>;  // (atom val,q',v',VV)
  // storing updated global row-max and row-sum
  // (1) - thr_id 0 ~ B_H-1 holds max of each row
  using FragARow = decltype(reduce<1>(FragA{}, sycl::plus<void>{}));
  // static_assert(is_same_v<decltype(FragA{}.shape()), float>, "dtype
  // mismatch");
  ...
  // Split-local accumulator tile over V dimension, e.g. (8, 128) when
  // V_SPLIT=4.
  ...
  // static_assert(is_same_v<decltype(SGTileShapeO{}), float>, "dtype
  // mismatch");
  ...
  // Power-of-2 D_QK strides (e.g., 512) cause all SIMD16 lanes to hit the same
  // SLM bank, resulting in serialized access. Padding by 64 elements makes
  // D_QK=512 use a 576-element row stride (1152 bytes), which is empirically
  // conflict-free (same as D_QK=576 kernel). static constexpr int SLM_STRIDE =
  // ((D_QK & (D_QK - 1)) == 0) ? D_QK + 64 : D_QK;
  ...
  void operator()(const Params& params, char* smem_buf) const {
    ...
    // using GmemLayoutO = Layout<Shape<int, Int<Traits::D_V_PER_SPLIT>>,
    // Stride<int, _1>>;

    auto* q_ptr = q + seq_idx * params.stride_q_s_q;
    auto q_layout =
        make_layout(make_shape(params.h_q, D_QK), make_stride(D_QK, _1{}));
    Tensor Q = make_tensor(make_gmem_ptr(q_ptr), q_layout);

    auto* out_ptr = out + seq_idx * params.h_q * params.d_v;
    auto o_layout = make_layout(
        make_shape(params.h_q, Traits::D_V), make_stride(Traits::D_V, _1{}));
    ...
    auto* gathered_v_ptr =
        gathered_k + seq_idx * params.stride_gathered_k_s_q + cur_v_start_idx;
    ...
    Tensor gO = local_tile(
        proxyO, TileShapeOut{}, make_coord(head_bid, v_split_idx));  // (h,d)
    ...
    //
    // (16,1,1,16)
    // static_assert(is_same_v<decltype(typename
    // Traits::TiledMMAQK::ThrLayoutVMNK{}), float>, "dtype mismatch");
    ...
    // using TensorO2D =
    // decltype(O(append<rank_v<decltype(O)>>(make_coord(_,_),0))); using
    // TensorO2D = decltype(O);
    // static_assert(is_same_v<decltype(TensorO2D{}.shape()), float>, "dtype
    // mismatch"); auto tiled_copy_O = (ReduceK{} > _1{}) ?
    // make_block_2d_copy_D_subtiled(mma_pv, ReduceFragA{}.tv_layout(),
    ...
    const int real_topk_length =
        HAVE_TOPK_LENGTH ? *(topk_length + seq_idx) : params.topk;

    const int* gathered_valid_mask =
        params.gathered_valid_mask + seq_idx * params.stride_gathered_mask_s_q;
    ...
    auto mask_rS = [&](int block_idx) {
      // for those invalid tokens, we need mask value to -INF for later softmax
      // calculation
      ...
      for (int i = 0; i < tSrS.size(); ++i) {
        ...
        bool is_valid = topk_idx < real_topk_length && topk_idx < params.topk &&
                        gathered_valid_mask[topk_idx];
        if (!is_valid) {
          // note that cannot use -INFINITY due to numerical issue when applying
          // exp2(prev_max-cur_max) later, use the lowest value instead
    ...
    auto reduce_L = [&]() {
      // refer to
      // https://github.com/intel/sycl-tla/blob/main/applications/flash_attention_v2/collective/xe_fmha_fwd_epilogue.hpp#L188
      if constexpr (ReduceK{} == _1{}) {
        ...
        /* Set up SLM tensors and partition A tiles among participating
         * subgroups */
        auto shape_A =
        ...
        auto sA = make_tensor(
            make_smem_ptr<ElementA>(&shared_storage.a_data),
            sA_layout);  // (q,v,rblk_dst,rblk_src,a_tile)
        auto sA_max = make_tensor(
            make_smem_ptr<ElementA>(&shared_storage.a_max_data),
            sA_row_layout);  // (q,rblk_dst,rblk_src,a_tile)
        auto sA_sum = make_tensor(
            make_smem_ptr<ElementA>(&shared_storage.a_sum_data),
            sA_row_layout);  // (q,rblk_dst,rblk_src,a_tile)
        ...
        if (active) {
          /* Read A/A_sum back from SLM, align scaling to new maxima, and
           * reduce. */
          clear(rA_sum);
    ...
    const int num_topk_blocks = HAVE_TOPK_LENGTH
                                    ? ceil_div(real_topk_length, Traits::B_TOPK)
                                    : ceil_div(params.topk, Traits::B_TOPK);
...
class SparsePrefillGatherKernel {
 public:
  ...
  static_assert(
      NUM_THREADS % SUBGROUP_SIZE == 0,
      "NUM_THREADS must be divisible by SUBGROUP_SIZE");
  static_assert(
      sizeof(PackedElementKV) == sizeof(ElementKV) * ELEMENTS_PER_LANE,
      "PackedElementKV must cover exactly one lane chunk");
  static_assert(
      D_QK % (SUBGROUP_SIZE * ELEMENTS_PER_LANE) == 0,
  ...
  void operator()(const Params& params, char* smem_buf) const {
    ...
    const int real_topk_length =
        HAVE_TOPK_LENGTH ? params.topk_length[seq_idx] : params.topk;

    const int* indices = params.indices + seq_idx * params.stride_indices_s_q;
    ElementKV* gathered_k =
        params.gathered_k + seq_idx * params.stride_gathered_k_s_q;
    int* gathered_valid_mask =
        params.gathered_valid_mask + seq_idx * params.stride_gathered_mask_s_q;
    const bool can_pack =
        params.stride_kv_s_kv % ELEMENTS_PER_LANE == 0 &&
        params.stride_gathered_k_topk % ELEMENTS_PER_LANE == 0 &&
        (reinterpret_cast<uintptr_t>(params.kv) &
         (sizeof(PackedElementKV) - 1)) == 0 &&
    ...
    for (int local_topk_idx = sg_id; local_topk_idx < B_TOPK;
      ...
      const int kv_row_offset =
          valid_token ? token_idx * params.stride_kv_s_kv : 0;
      for (int d_base = lane_id * ELEMENTS_PER_LANE; d_base < D_QK;
           d_base += SUBGROUP_SIZE * ELEMENTS_PER_LANE) {
        const int gathered_offset = gathered_row_offset + d_base;
        if (can_pack) {
          const PackedElementKV value =
              valid_token ? *reinterpret_cast<const PackedElementKV*>(
                                params.kv + kv_row_offset + d_base)
                          : PackedElementKV(0);
          *reinterpret_cast<PackedElementKV*>(gathered_k + gathered_offset) =
              value;
        } else {
          CUTE_UNROLL
          for (int vec_offset = 0; vec_offset < ELEMENTS_PER_LANE;
               ++vec_offset) {
            const int d_idx = d_base + vec_offset;
            const int scalar_gathered_offset = gathered_row_offset + d_idx;
            const ElementKV value =
                valid_token ? params.kv[kv_row_offset + d_idx] : ElementKV(0);
    ...
    for (int local_topk_idx = thr_id; local_topk_idx < B_TOPK;
         local_topk_idx += NUM_THREADS) {
      const int topk_idx = topk_base + local_topk_idx;
      if (topk_idx < params.topk) {
        const bool in_range = topk_idx < real_topk_length;
        const int token_idx = in_range ? indices[topk_idx] : -1;
        gathered_valid_mask[topk_idx] =
            (int)(token_idx >= 0 && token_idx < params.s_kv);
      }
    }
  }
};
...
template <int D_QK, bool HAVE_TOPK_LENGTH>
void launch_sparse_mla_prefill_gather_kernel(
    const XPUSparseAttnFwdParams& params) {
...
void launch_mla_prefill_fwd_kernel_policy(
    const XPUSparseAttnFwdParams& params) {
  ...
  using Kernel =
      DensePrefillFwdKernel<D_QK, HAVE_TOPK_LENGTH, HAS_ATTN_SINK, Traits>;
...
void launch_mla_prefill_fwd_kernel(const XPUSparseAttnFwdParams& params) {
  switch (sparse_mla_prefill_select_b_h(params)) {
    case 8:
      launch_mla_prefill_fwd_kernel_policy<
          D_QK,
          HAVE_TOPK_LENGTH,
          HAS_ATTN_SINK,
          8>(params);
...
template <int D_QK, bool HAVE_TOPK_LENGTH, bool HAS_ATTN_SINK>
void launch_sparse_mla_prefill_fwd_kernel(
    const XPUSparseAttnFwdParams& params) {
