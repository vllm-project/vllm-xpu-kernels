...
struct KernelTraits {
  ...
  using SubgroupLayoutQK = conditional_t<
      (B_H > 16),
  ...
  using SubgroupLayoutPV = conditional_t<
      (B_H > 16),
      Layout<Shape<Int<NUM_SUBGROUPS>, _1, _1>>,
  ...
  using SmemTileLayoutK = Layout<
      Shape<Int<B_TOPK>, Int<HEAD_DIM_TILE_SIZE>>,
      Stride<Int<HEAD_DIM_TILE_SIZE>, _1>>;
  using SmemTileLayoutV = Layout<
      Shape<Int<HEAD_DIM_TILE_SIZE>, Int<B_TOPK>>,
      Stride<Int<B_TOPK>, _1>>;

  constexpr static int SGTileQ =
      get<0>(shape_div(TileShapeQK{}, shape(SubgroupLayoutQK{})))();
  // bf16 dpas m8n16k16
  // (8, 128, 64) / ((8, 16, 16) * (1, 16, 1)) = (1, 1, 4) iterations per
  // subgroup
  constexpr static int MAX_M_DPAS = 8;
  using MMAOperation =
      XE_DPAS_TT<cute::gcd(SGTileQ, MAX_M_DPAS), float, bfloat16_t>;
...
class SparseDecodeGatherDequantKernel {
 public:
  ...
  static_assert(
      D_QK == 512,
      "packed fp8 sparse decode currently supports logical D_QK=512");
  static_assert(
      D_QK % SUBGROUP_SIZE == 0, "D_QK must be divisible by SUBGROUP_SIZE");
  static_assert(
      D_QK == SPARSE_MLA_FP8_NOPE_BYTES + SPARSE_MLA_FP8_ROPE_DIM,
      "logical D_QK must match packed fp8 NoPE + RoPE dimensions");
  static_assert(
      SPARSE_MLA_FP8_NOPE_BYTES % FP8_VALUES_PER_PACK == 0,
  ...
  static_assert(
      SPARSE_MLA_FP8_NOPE_BYTES / 64 ==
          SPARSE_MLA_FP8_SCALE_BYTES_PER_TOKEN - 1,
      "only the first seven scale bytes are valid for 448 NoPE values");
  static_assert(
      sizeof(PackedElement) == FP8_VALUES_PER_PACK,
      "PackedElement must cover one fp8 lane chunk");
  static_assert(
      sizeof(PackedElement) ==
          sizeof(cutlass::bfloat16_t) * BF16_VALUES_PER_PACK,
  ...
  static float e8m0_to_float(uint8_t scale_byte) {
    return sycl::native::exp2(
        static_cast<float>(static_cast<int>(scale_byte) - 127));
  }

  CUTLASS_DEVICE
  static uint16_t
  fp8_e4m3_scaled_to_bf16_bits(uint8_t fp8_byte, uint8_t scale_byte) {
  ...
  static void store_dequantized_token_scalar(
    ...
    for (int n = 0; n < NUM_VALS_PER_THREAD; ++n) {
      ...
      if (valid_token && dim_idx < SPARSE_MLA_FP8_NOPE_BYTES) {
        ...
        auto fp8_val = *reinterpret_cast<const cutlass::float_e4m3_t*>(
            token_data + dim_idx);
        kv_val = cutlass::bfloat16_t(static_cast<float>(fp8_val) * scale);
      } else if (valid_token) {
        const auto* rope_ptr = reinterpret_cast<const cutlass::bfloat16_t*>(
            token_data + SPARSE_MLA_FP8_NOPE_BYTES);
  ...
  static void store_dequantized_token_packed(
    ...
    for (int d_base = lane_id * FP8_VALUES_PER_PACK;
      ...
      if (valid_token) {
        const PackedElement packed_fp8 =
            *reinterpret_cast<const PackedElement*>(token_data + d_base);
        const uint8_t scale_byte = token_scales[d_base / 64];
        CUTE_UNROLL
        for (int vec_offset = 0; vec_offset < FP8_VALUES_PER_PACK;
             ++vec_offset) {
          const uint8_t fp8_byte =
              static_cast<uint8_t>(packed_fp8 >> (8 * vec_offset));
          const uint16_t bf16_bits =
              fp8_e4m3_scaled_to_bf16_bits(fp8_byte, scale_byte);
          if (vec_offset < BF16_VALUES_PER_PACK) {
            packed_lo |= PackedElement(bf16_bits) << (16 * vec_offset);
          } else {
            packed_hi |= PackedElement(bf16_bits)
                         << (16 * (vec_offset - BF16_VALUES_PER_PACK));
          }
        }
      }
      *reinterpret_cast<PackedElement*>(gathered_row + d_base) = packed_lo;
      *reinterpret_cast<PackedElement*>(
          gathered_row + d_base + BF16_VALUES_PER_PACK) = packed_hi;
    }
  ...
  static void store_dequantized_token(
    ...
    if (valid_token) {
      ...
      token_data = active_kv + block_idx * active_stride_kv_block +
                   rel_idx * SPARSE_MLA_FP8_DATA_BYTES_PER_TOKEN;
      token_scales =
          active_kv + block_idx * active_stride_kv_block +
          active_page_block_size * SPARSE_MLA_FP8_DATA_BYTES_PER_TOKEN +
          rel_idx * SPARSE_MLA_FP8_SCALE_BYTES_PER_TOKEN;
    }

    if (can_pack) {
      store_dequantized_token_packed(
          gathered_row, token_data, token_scales, valid_token, lane_id);
    } else {
      store_dequantized_token_scalar(
          gathered_row, token_data, token_scales, valid_token, lane_id);
    }
  }
  ...
  void operator()(const Params& params, char* smem_buf) const {
    ...
    auto* gathered_k = params.gathered_k +
                       batch_idx * params.stride_gathered_k_b +
                       seq_idx * params.stride_gathered_k_s_q;
    auto* gathered_valid_mask = params.gathered_valid_mask +
                                batch_idx * params.stride_gathered_mask_b +
                                seq_idx * params.stride_gathered_mask_s_q;
    const int* main_indices = params.indices +
                              batch_idx * params.stride_indices_b +
                              seq_idx * params.stride_indices_s_q;
    ...
    auto resolve_topk_length =
        [&](const int* topk_length_ptr, int topk, int stride_b) {
    ...
    const int main_topk_length = resolve_topk_length(
        params.topk_length, params.topk, params.stride_topk_length_b);
    const int extra_topk_length = resolve_topk_length(
        params.extra_topk_length,
        params.extra_topk,
        params.stride_extra_topk_length_b);

    for (int local_topk_idx = sg_id; local_topk_idx < B_TOPK;
         local_topk_idx += NUM_SUBGROUPS) {
      ...
      int active_num_blocks =
          is_extra ? params.extra_num_blocks : params.num_blocks;
      int active_page_block_size =
          is_extra ? params.extra_page_block_size : params.page_block_size;
      int active_stride_kv_block =
          is_extra ? params.stride_extra_kv_block : params.stride_kv_block;
      const bool can_pack =
          params.stride_gathered_k_topk % BF16_VALUES_PER_PACK == 0 &&
          active_stride_kv_block % sizeof(PackedElement) == 0 &&
          (reinterpret_cast<uintptr_t>(active_kv) &
           (sizeof(PackedElement) - 1)) == 0 &&
          (reinterpret_cast<uintptr_t>(gathered_k) &
           (sizeof(PackedElement) - 1)) == 0;

      bool valid_token = false;
      int token_idx = -1;
      if (active_indices != nullptr && active_kv != nullptr &&
          range_topk_idx < active_topk && range_topk_idx < active_topk_length) {
        token_idx = active_indices[range_topk_idx];
        valid_token = token_idx >= 0 &&
                      token_idx < active_num_blocks * active_page_block_size;
      }

      cutlass::bfloat16_t* gathered_row =
          gathered_k + topk_idx * params.stride_gathered_k_topk;
...
class DenseDecodeFwdKernel {
 public:
  ...
  using SingleFragA =
      decltype(TiledMMAPV{}.get_slice(0).partition_sg_fragment_C(
          make_identity_tensor(select<0, 1>(TiledMMAPV{}.tile_mnk()))));
  static_assert(
      Traits::D_V_PER_SPLIT % get<1>(TileShapePV{}) == 0,
      "D_V_PER_SPLIT must be divisible by TileShapePV N dimension");
  using FragA = expand_sg_fragment_t<
      SingleFragA,
      1,
      Traits::D_V_PER_SPLIT / get<1>(TileShapePV{})>;
  ...
  using GmemLayoutO = Layout<
      Shape<int, Int<Traits::D_V_PER_SPLIT>>,
      Stride<Int<Traits::D_V>, _1>>;
  using TensorO2D =
      decltype(make_tensor(make_gmem_ptr((ElementO*)nullptr), GmemLayoutO{}));
  ...
  void operator()(const Params& params, char* smem_buf) const {
    ...
    auto* q_ptr =
        q + batch_idx * params.stride_q_b + seq_idx * params.stride_q_s_q;
    auto q_layout = make_layout(
        make_shape(params.h_q, D_QK), make_stride(params.stride_q_h_q, _1{}));
    Tensor Q = make_tensor(make_gmem_ptr(q_ptr), q_layout);

    auto* out_ptr =
        out + batch_idx * params.stride_o_b + seq_idx * params.stride_o_s_q;
    auto o_layout = make_layout(
        make_shape(params.h_q, Traits::D_V),
        make_stride(params.stride_o_h_q, _1{}));
    Tensor O = make_tensor(make_gmem_ptr(out_ptr), o_layout);

    const auto* gathered_k_ptr = params.gathered_k +
                                 batch_idx * params.stride_gathered_k_b +
                                 seq_idx * params.stride_gathered_k_s_q;
    const auto* gathered_v_ptr = gathered_k_ptr + cur_v_start_idx;
    auto gathered_k_layout = make_layout(
        make_shape(params.gathered_topk, D_QK),
        make_stride(params.stride_gathered_k_topk, _1{}));
    auto gathered_v_layout = make_layout(
        make_shape(Traits::D_V_PER_SPLIT, params.gathered_topk),
        make_stride(_1{}, params.stride_gathered_k_topk));
    Tensor K = make_tensor(
        make_gmem_ptr(const_cast<ElementKV*>(gathered_k_ptr)),
        gathered_k_layout);
    Tensor V = make_tensor(
        make_gmem_ptr(const_cast<ElementKV*>(gathered_v_ptr)),
        gathered_v_layout);
    ...
    constexpr int V_TILE_PER_SPLIT =
        Traits::D_V_PER_SPLIT / get<1>(TileShapePV{});
    auto tile_shape_v =
        make_shape(Int<Traits::D_V_PER_SPLIT>{}, get<2>(TileShapePV{}));

    Tensor gQ = local_tile(
        proxyQ, TileShapeQK{}, make_coord(head_bid, _, _), Step<_1, X, _1>{});
    Tensor gK = local_tile(
        proxyK, TileShapeQK{}, make_coord(_, _, _), Step<X, _1, _1>{});
    Tensor gV = local_tile(proxyV, tile_shape_v, make_coord(0, _));
    Tensor gV_split =
        local_tile(gV, TileShapePV{}, make_coord(_, _, 0), Step<X, _1, _1>{});
    Tensor gO =
        local_tile(proxyO, TileShapeOut{}, make_coord(head_bid, v_split_idx));
    ...
    const int* gathered_valid_mask_ptr =
        gathered_valid_mask + batch_idx * params.stride_gathered_mask_b +
        seq_idx * params.stride_gathered_mask_s_q;
    auto mask_rS = [&](int block_idx) {
      ...
      for (int i = 0; i < tSrS.size(); ++i) {
        ...
        bool is_valid = topk_idx < params.gathered_topk &&
                        gathered_valid_mask_ptr[topk_idx];
        if (!is_valid) {
    ...
    auto online_softmax_and_rescale_o = [&](int block_idx) {
      ...
      CUTE_UNROLL
      for (int i = 0; i < tSrS.size(); i++) {
        tSrS(i) = sycl::native::exp2(
            params.sm_scale_div_log2 * tSrS(i) - broadcast<0>(tA_max, tSrS, i));
      }
    ...
    auto reduce_L = [&]() {
      if constexpr (ReduceK{} == _1{}) {
        ...
        auto thr_vak = group<1, 3>(mma_pv.get_thr_layout_vmnk())
        ...
        auto sA = make_tensor(
            make_smem_ptr<ElementA>(&shared_storage.a_data), sA_layout);
        auto sA_max = make_tensor(
            make_smem_ptr<ElementA>(&shared_storage.a_max_data), sA_row_layout);
        auto sA_sum = make_tensor(
            make_smem_ptr<ElementA>(&shared_storage.a_sum_data), sA_row_layout);
    ...
    auto final_rescale_and_store_o = [&]() {
      ...
      constexpr int valid_tid_per_sg =
          Traits::B_H / get<0>(typename Traits::SubgroupLayoutQK{}.shape());
      static_assert(
          valid_tid_per_sg <= Traits::SUBGROUP_SIZE,
          "valid_tid_per_sg must be less than or equal to SUBGROUP_SIZE");
      if (v_split_idx == 0 && tid_in_sg < valid_tid_per_sg &&
          sg_id * valid_tid_per_sg < Traits::B_H) {
        int local_head_idx = sg_id * valid_tid_per_sg + tid_in_sg;
        int head_idx = cur_head_start_idx + local_head_idx;
        if (head_idx < params.h_q) {
          int lse_idx = batch_idx * params.stride_lse_b +
                        seq_idx * params.stride_lse_s_q + head_idx;
      ...
      if constexpr (HAS_ATTN_SINK) {
        if constexpr (ReduceK{} > 1) {
          auto thr_vak = group<1, 3>(mma_pv.get_thr_layout_vmnk())
                             .get_flat_coord(assert_uniform(thr_id));
          int reduce_head_offset =
              (get<2>(thr_vak) / ReduceSGV{}) * get<0>(SGTileShapeO{});

          CUTE_UNROLL
          for (int i = 0; i < rA.size(); ++i) {
            auto global_exp_sum = broadcast<0>(rA_sum, rA, i);
            ElementA final_rescale = global_exp_sum != 0
                                         ? ElementA(1) / global_exp_sum
                                         : ElementA(0);
            int local_head_idx = get<0>(rA.tv_layout()(0, i));
            int head_idx =
                cur_head_start_idx + reduce_head_offset + local_head_idx;
            if (head_idx < params.h_q) {
              ...
              ElementA sink_exp_sum = sycl::native::exp2(
                  static_cast<ElementA>(
                      attn_sink_val * FLASH_NAMESPACE::LOG_2_E) -
                  global_max);
              ElementA global_exp_sum_with_sink = global_exp_sum + sink_exp_sum;
              final_rescale = global_exp_sum_with_sink != 0
                                  ? ElementA(1) / global_exp_sum_with_sink
                                  : ElementA(0);
            }
            rA(i) *= final_rescale;
          }
        } else {
          if (tid_in_sg < valid_tid_per_sg &&
              sg_id * valid_tid_per_sg < Traits::B_H) {
            int local_head_idx = sg_id * valid_tid_per_sg + tid_in_sg;
            int head_idx = cur_head_start_idx + local_head_idx;
            if (head_idx < params.h_q) {
...
void launch_sparse_mla_decode_fp8_fwd_kernel_policy(
    const XPUSparseDecodeAttnFwdParams& params) {
  using Traits = KernelTraits<B_H>;

  TORCH_CHECK(params.h_kv == 1, "h_kv must be 1");
  TORCH_CHECK(params.h_q > 0, "h_q must be > 0");
  TORCH_CHECK(
      params.d_qk == D_QK, "Invalid d_qk for this kernel instantiation");
  TORCH_CHECK(params.d_v == Traits::D_V, "d_v must match KernelTraits::D_V");
  TORCH_CHECK(
      params.gathered_topk == params.topk + params.extra_topk,
      "gathered_topk must equal topk + extra_topk");

  using GatherDequantKernel =
      SparseDecodeGatherDequantKernel<D_QK, HAVE_TOPK_LENGTH>;
  ...
  if (params.gathered_topk > 0) {
    ...
    dim3 grid(
        params.b * params.s_q,
        ceil_div(params.gathered_topk, GatherDequantKernel::B_TOPK),
        1);
    ...
    compat::experimental::kernel_properties kernel_props{
        sycl::ext::oneapi::experimental::sub_group_size<
            GatherDequantKernel::SUBGROUP_SIZE>,
    ...
    auto event = compat::experimental::launch<
        cutlass::device_kernel<GatherDequantKernel>,
        GatherDequantKernel>(
        policy, params.queue, static_cast<SparseAttnDecodeParams>(params));
  }

  {
    dim3 block(Traits::NUM_THREADS, 1, 1);
    dim3 grid(
        ceil_div(params.h_q, Traits::B_H) * params.s_q * params.b,
        Traits::V_SPLIT,
        1);
    ...
    auto event = compat::experimental::
        launch<cutlass::device_kernel<DenseKernel>, DenseKernel>(
            policy, params.queue, static_cast<SparseAttnDecodeParams>(params));
  }
}

inline int
sparse_mla_decode_select_b_h(const XPUSparseDecodeAttnFwdParams& params) {
  TORCH_CHECK(params.h_q > 0, "h_q must be > 0");
...
void launch_sparse_mla_decode_fp8_fwd_kernel(
    const XPUSparseDecodeAttnFwdParams& params) {
  switch (sparse_mla_decode_select_b_h(params)) {
    case 8:
      launch_sparse_mla_decode_fp8_fwd_kernel_policy<
          D_QK,
          HAS_TOPK_LENGTH,
          HAS_ATTN_SINK,
          8>(params);
