...
struct SparseAttnFwdParams {
  ...
  int stride_gathered_k_s_q;
  int stride_gathered_k_topk;
...
struct SparseAttnDecodeParams {
  ...
  uint8_t* __restrict__ kv;       // packed fp8 KV cache, [num_blocks,
                                  // page_block_size, h_kv=1, head_bytes]
  int* __restrict__ indices;      // [b, s_q, topk]
  int* __restrict__ topk_length;  // [b], may be nullptr
  float* __restrict__ attn_sink;  // [h_q], may be nullptr
  cutlass::bfloat16_t* __restrict__ gathered_k;  // [b, s_q, gathered_topk,
                                                 // d_qk]
  ...
  int stride_topk_length_b,
      stride_topk_length_s_q;  // stride_topk_length_s_q is unused for decode
                               // (1D topk_length)
  ...
  int stride_extra_topk_length_b,
      stride_extra_topk_length_s_q;  // stride_extra_topk_length_s_q is unused
                                     // for decode (1D extra_topk_length)
  ...
  void* __restrict__ tile_scheduler_metadata_ptr;  // unused by no-split sparse
                                                   // decode for now
