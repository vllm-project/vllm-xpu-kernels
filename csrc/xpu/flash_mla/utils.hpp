...
#define CHECK_OPTIONAL_SHAPE(x, ...)                         \
  if (x.has_value()) {                                       \
    TORCH_CHECK(                                             \
        x.value().sizes() == at::IntArrayRef({__VA_ARGS__}), \
        #x " must have shape (" #__VA_ARGS__ ")");           \
  }
...
// Return (PtrT*)tensor.data_ptr() if the tensor has a backend storage, nullptr
// otherwise
...
// Return (PtrT*)tensor.data_ptr() if tensor_or_opt has value and points to a
// valid tensor, return nullptr otherwise
