...
std::vector<at::Tensor> flash_mla_sparse_attn_prefill_impl(
  ...
  if (attn_sink.has_value()) {
    TORCH_CHECK(
        attn_sink.value().dtype() == torch::kFloat32,
        "attn_sink must be float32");
    CHECK_DEVICE(attn_sink.value());
  }
  if (topk_length.has_value()) {
    TORCH_CHECK(
        topk_length.value().dtype() == torch::kInt32,
  ...
  TORCH_CHECK(
      false, "Sparse MLA prefill requires VLLM_XPU_ENABLE_XE2 build option.");
#endif
  ...
  at::Tensor out =
      output.has_value() ? output.value() : torch::empty({s_q, h_q, d_v}, opts);
  ...
  at::Tensor gathered_valid_mask =
      torch::empty({s_q, topk}, opts.dtype(torch::kInt));
  ...
  auto launch_prefill = [&](auto head_dim_tag, auto topk_tag, auto sink_tag) {
    ...
    launch_sparse_mla_prefill_fwd_kernel<
        HEAD_DIM_QK,
        HAVE_TOPK_LENGTH,
        HAS_ATTN_SINK>(params);
  };
  ...
  if (params.d_qk == 512) {
    ...
    TORCH_CHECK(
        false, "Unsupported sparse MLA prefill head dim: ", params.d_qk);
  }
...
flash_mla_sparse_attn_decode_impl(
    ...
    std::optional<at::Tensor>&
        tile_scheduler_metadata,  // num_sm_parts x (DecodingSchedMetaSize/4)
  ...
  TORCH_CHECK(
      kv.dim() == 4,
      "kv must be 4D [num_blocks, page_block_size, h_kv, head_bytes]");
  ...
  TORCH_CHECK(
      indices.size(0) == b && indices.size(1) == s_q,
      "indices batch/query dims must match q");
  TORCH_CHECK(h_kv == 1, "sparse MLA decode currently requires h_kv == 1");
  TORCH_CHECK(
      d_qk == 512, "packed fp8 sparse MLA decode requires q head dim 512");
  TORCH_CHECK(d_v == 512, "d_v must be 512");
  TORCH_CHECK(
      head_bytes == SPARSE_MLA_FP8_HEAD_BYTES,
      "kv last dim must be packed head_bytes=584");
  TORCH_CHECK(indices.dtype() == torch::kInt32, "indices must be int32");
  TORCH_CHECK(
      kv.dtype() == at::ScalarType::Float8_e4m3fn,
      "kv must be float8_e4m3fn packed storage");
  TORCH_CHECK(
      q.dtype() == torch::kBFloat16,
      "sparse MLA decode requires q dtype bfloat16");
  ...
  if (topk_length.has_value()) {
    ...
    TORCH_CHECK(
        topk_length.value().size(0) == b, "topk_length shape must match [b]");
    TORCH_CHECK(
        topk_length.value().dtype() == torch::kInt32,
        "topk_length must be int32");
    CHECK_DEVICE(topk_length.value());
  ...
  if (attn_sink.has_value()) {
    TORCH_CHECK(
        attn_sink.value().dim() == 1 && attn_sink.value().size(0) == h_q,
        "attn_sink must have shape [h_q]");
    TORCH_CHECK(
        attn_sink.value().dtype() == torch::kFloat32,
        "attn_sink must be float32");
    CHECK_DEVICE(attn_sink.value());
  ...
  if (tile_scheduler_metadata.has_value()) {
    TORCH_CHECK(
        false,
        "sparse fp8 decode splitKV scheduler metadata is not supported in the "
        "no-split XPU implementation yet");
  }
  if (num_splits.has_value()) {
    TORCH_CHECK(
        false,
        "sparse fp8 decode splitKV num_splits is not supported in the no-split "
        "XPU implementation yet");
  }
  ...
  if (extra_kv.has_value()) {
    TORCH_CHECK(
        ...
        "extra_indices must be provided when extra_kv is provided");
    TORCH_CHECK(
        extra_kv.value().dim() == 4,
        "extra_kv must be 4D [num_blocks, page_block_size, h_kv, head_bytes]");
    TORCH_CHECK(
        extra_kv.value().size(2) == 1 &&
            extra_kv.value().size(3) == SPARSE_MLA_FP8_HEAD_BYTES,
        "extra_kv must have h_kv=1 and head_bytes=584");
    TORCH_CHECK(
        extra_kv.value().dtype() == at::ScalarType::Float8_e4m3fn,
        "extra_kv must be float8_e4m3fn packed storage");
    CHECK_DEVICE(extra_kv.value());
    TORCH_CHECK(
        extra_indices.value().dim() == 3,
        "extra_indices must be 3D [b, s_q, extra_topk]");
    TORCH_CHECK(
        extra_indices.value().size(0) == b &&
            extra_indices.value().size(1) == s_q,
        "extra_indices batch/query dims must match q");
    TORCH_CHECK(
        extra_indices.value().dtype() == torch::kInt32,
  ...
  if (extra_topk_length.has_value()) {
    ...
    TORCH_CHECK(
        extra_topk_length.value().dim() == 1,
        "extra_topk_length must be 1D [b]");
    TORCH_CHECK(
        extra_topk_length.value().size(0) == b,
        "extra_topk_length shape must match [b]");
    TORCH_CHECK(
        extra_topk_length.value().dtype() == torch::kInt32,
        "extra_topk_length must be int32");
    CHECK_DEVICE(extra_topk_length.value());
  }

#if !defined(VLLM_XPU_ENABLE_XE2)
  TORCH_CHECK(
      false, "Sparse MLA decode requires VLLM_XPU_ENABLE_XE2 build option.");
#endif
  ...
  if (out.has_value()) {
    TORCH_CHECK(
        out.value().sizes() == torch::IntArrayRef({b, s_q, h_q, d_v}),
        "out must have shape [b, s_q, h_q, d_v]");
    TORCH_CHECK(
        out.value().dtype() == torch::kBFloat16, "out must be bfloat16");
  ...
  at::Tensor lse =
      torch::empty({b, s_q, h_q}, q.options().dtype(torch::kFloat));
  const int gathered_topk = topk + extra_topk;
  at::Tensor gathered_k = torch::empty({b, s_q, gathered_topk, d_qk}, out_opts);
  at::Tensor gathered_valid_mask =
      torch::empty({b, s_q, gathered_topk}, q.options().dtype(torch::kInt32));
  ...
  params.gathered_k =
      reinterpret_cast<cutlass::bfloat16_t*>(gathered_k.data_ptr());
  params.gathered_valid_mask =
      reinterpret_cast<int*>(gathered_valid_mask.data_ptr());
  ...
  params.extra_kv =
      extra_kv.has_value()
          ? reinterpret_cast<uint8_t*>(extra_kv.value().data_ptr())
          : nullptr;
  params.extra_indices =
      extra_indices.has_value()
          ? reinterpret_cast<int*>(extra_indices.value().data_ptr())
          : nullptr;
  ...
  params.stride_topk_length_b =
      topk_length.has_value()
          ? int64_stride_to_int(topk_length.value().stride(0))
          : 0;
  ...
  params.stride_gathered_mask_b =
      int64_stride_to_int(gathered_valid_mask.stride(0));
  params.stride_gathered_mask_s_q =
      int64_stride_to_int(gathered_valid_mask.stride(1));
  ...
  params.stride_extra_kv_block =
      extra_kv.has_value() ? int64_stride_to_int(extra_kv.value().stride(0))
                           : 0;
  params.stride_extra_kv_row =
      extra_kv.has_value() ? int64_stride_to_int(extra_kv.value().stride(1))
                           : 0;
  params.stride_extra_kv_head =
      extra_kv.has_value() ? int64_stride_to_int(extra_kv.value().stride(2))
                           : 0;
  params.stride_extra_indices_b =
      extra_indices.has_value()
          ? int64_stride_to_int(extra_indices.value().stride(0))
          : 0;
  params.stride_extra_indices_s_q =
      extra_indices.has_value()
          ? int64_stride_to_int(extra_indices.value().stride(1))
          : 0;
  params.stride_extra_topk_length_b =
      extra_topk_length.has_value()
          ? int64_stride_to_int(extra_topk_length.value().stride(0))
          : 0;
  ...
  auto launch_decode = [&](auto topk_tag, auto sink_tag) {
    constexpr bool HAVE_TOPK_LENGTH = decltype(topk_tag)::value;
    constexpr bool HAS_ATTN_SINK = decltype(sink_tag)::value;
    launch_sparse_mla_decode_fp8_fwd_kernel<
        512,
        HAVE_TOPK_LENGTH,
        HAS_ATTN_SINK>(params);
  };
