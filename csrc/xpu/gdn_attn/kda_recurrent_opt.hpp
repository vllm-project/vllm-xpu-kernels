...
// Sequential (token-at-a-time) KDA gated delta rule, selected by
// `VLLM_XPU_KDA_RECURRENT_MODE=opt` and used by `auto` whenever the chunked
// Xe2 pipeline declines the batch. It is the only backend that covers every
// supported shape: all head dimensions, all activation and state dtypes, and
// the spec-decode batch layout.
//
// The recurrence itself is the textbook one (see kda_gate.hpp for the gate):
// per token, decay the state per key channel, read it with `k`, write back the
// rank-1 delta scaled by `beta`, then read the result out with `q`. What makes
// this variant fast is how that maps onto the hardware:
//
//   * `head_dim` is a compile-time constant (`KBucket * sub_group_size`), so
//     every offset computation constant-folds instead of emitting runtime
...
//   * `Mode` drops the per-sequence token loop for pure decode and hoists
//     loop-invariant work (dt_bias, head decay coefficient) out of it.
//
// Transcendentals come from `sycl::native::*`; the results feed exp(g) and are
// consumed at activation precision, so the extra ulps do not matter.
