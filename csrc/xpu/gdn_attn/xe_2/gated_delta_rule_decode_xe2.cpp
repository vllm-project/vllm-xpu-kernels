...
void gated_delta_rule_decode_xe2(
  ...
  TORCH_CHECK(
      cache_indices.is_contiguous(), "cache_indices must be contiguous");
  TORCH_CHECK(
      cache_indices.dtype() == torch::kInt32, "cache_indices must be int32");
  TORCH_CHECK(cache_indices.dim() == 1, "cache_indices must be 1D");
  TORCH_CHECK(
      cache_indices.size(0) == batch_size,
      "cache_indices size must match batch_size");
  if (has_initial_state.has_value()) {
    ...
    TORCH_CHECK(has_initial_state->dim() == 1, "has_initial_state must be 1D");
    TORCH_CHECK(
...
void gated_delta_rule_decode_xe2_spec(
  ...
  TORCH_CHECK(
      cache_indices.size(0) == num_spec_decodes,
      "cache_indices size(0) must match num_spec_decodes");
  TORCH_CHECK(
