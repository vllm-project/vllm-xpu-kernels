...
CUTE_DEVICE void chunk_fwd_o_kernel(
  ...
  for (int batch_id = 0; batch_id < batch_size; ++batch_id) {
    ...
    for (int chunk_id = 0; chunk_id < current_chunks; ++chunk_id) {
      ...
      const int out_seq_offset = seq_start_offset + chunk_id * chunk_size;
      const int virtual_seq_offset = (pre_chunks + chunk_id) * chunk_size;
      ...
      // ------------------------------------------------------------------
      // Part 1: Compute current chunk output O using previous state S_prev.
      // ------------------------------------------------------------------

      // If S_prev exists, first apply recurrent correction:
      //   U <- U - W * S_prev
      // This keeps U in-place and avoids extra global buffers.
      if (has_prev_state) {
        for (int dv = 0; dv < head_v_dim / chunk_size; ++dv) {
          Tensor gU_C =
              local_tile(cU, wg_tile, make_coord(0, dv, 0), Step<_1, _1, X>{});
      ...
      // Write output O for this chunk:
      //   with S_prev: O = exp(g) * (Q * S_prev) + O2 * U
      //   no S_prev : O = O2 * U
      if (has_prev_state) {
        for (int dv = 0; dv < head_v_dim / chunk_size; ++dv) {
          Tensor gO_C =
