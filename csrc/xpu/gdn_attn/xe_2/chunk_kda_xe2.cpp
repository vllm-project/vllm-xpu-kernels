...
#define KDA_CHUNK_LAUNCH(scalar_t, state_t)                       \
  ...
  // Only the two combinations the dispatcher can actually produce are
  // instantiated. `launch_kda_recurrent` enters this path for bf16 activations
  // only, and `validate_recurrent_inputs` restricts the state cache to fp32 or
  // the activation dtype. Instantiating the other four would quadruple the
  // compile time of the seven-stage CuTe pipeline for code nothing can reach.
  TORCH_CHECK(
      dtype == at::kBFloat16,
      "chunk_kda_xe2 activations must be bfloat16, got ",
      dtype);
  if (recurrent_state.scalar_type() == at::kFloat) {
    KDA_CHUNK_LAUNCH(cutlass::bfloat16_t, float);
  } else if (recurrent_state.scalar_type() == at::kBFloat16) {
    KDA_CHUNK_LAUNCH(cutlass::bfloat16_t, cutlass::bfloat16_t);
  } else {
    TORCH_CHECK(
        false,
        "chunk_kda_xe2 recurrent_state dtype must be float32 or bfloat16, "
        "got ",
