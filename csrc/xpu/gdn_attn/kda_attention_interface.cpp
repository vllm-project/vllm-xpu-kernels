...
// Kimi Delta Attention (KDA) ops for XPU.
//
// Three entry points are registered on `_xpu_C` (see xpu/torch_bindings.cpp):
//
//   kda_causal_conv1d    depthwise causal conv1d + SiLU over the q/k/v
//                        projections, updating `conv_state`; returns {q, k, v}
//   kda_gated_delta_rule the KDA recurrence over those {q, k, v}, writing
//                        `core_attn_out` and updating `recurrent_state`
//   kda_attention        the two above chained, for callers that do not need
//                        the intermediate {q, k, v}
//
// Both stages take *raw* projection outputs: the kernels apply the gate
// activation to `raw_gate` and `sigmoid` to `raw_beta` themselves (kda_gate.hpp
// explains why).
//
// The recurrence has two backends:
//
//   opt   `recurrent_kda_opt_kernel` (kda_recurrent_opt.hpp). Sequential over
//         tokens. Covers every shape, dtype and batch layout, so it is the
//         fallback whenever `chunk` declines a batch.
//   chunk `chunk_kda_xe2` (xe_2/chunk_kda_xe2.cpp). XMX/DPAS chunked pipeline;
//         much faster on prefill but only applicable to non-spec batches on
//         Xe2 with bf16 activations and `head_dim % chunk_size == 0`.
//
// Environment variables (all read once and cached):
//
//   VLLM_XPU_KDA_RECURRENT_MODE       auto (default) | opt | chunk
//   VLLM_XPU_KDA_CHUNK_MIN_SEQLEN     `auto` keeps short batches on `opt`
//   VLLM_XPU_KDA_CHUNK_MAX_WORKSPACE_MB  scratch budget for the chunk pipeline
//   VLLM_XPU_KDA_CHUNK_STRICT         turn the decay-range fallback into an
//                                     error instead of a silent retry on `opt`

namespace {

// ---------------------------------------------------------------------------
// Backend selection and tuning knobs
// ---------------------------------------------------------------------------

// Backend selection for the KDA recurrent (gated delta rule) stage.
//   auto  - heuristic selection (default)
//   opt   - always use the sequential `recurrent_kda_opt_kernel`
//   chunk - prefer the chunked XMX pipeline for non-spec batches, falling back
//           to `opt` for the batches it cannot handle
enum class KdaRecurrentBackend { Auto, Optimized, Chunk };
...
KdaRecurrentBackend parse_kda_recurrent_backend() {
  ...
  TORCH_CHECK(
      value == "auto",
      "VLLM_XPU_KDA_RECURRENT_MODE must be one of auto/opt/chunk, got ",
      value,
      ". The unvectorised 'recurrent' reference backend has been removed; "
      "'opt' supersedes it and covers every supported shape and dtype.");
  return KdaRecurrentBackend::Auto;
}
...
// ---------------------------------------------------------------------------
// Stage 1 dispatch: causal conv1d
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Stage 2 dispatch: gated delta rule recurrence
// ---------------------------------------------------------------------------
...
// Runs the recurrence for one activation dtype: the chunked pipeline first if
// it accepts the non-spec half of the batch, then `opt` for whatever is left.
template <typename T>
void launch_kda_recurrent(
    ...
    int64_t num_actual_tokens) {
  const int non_spec_batch_size = num_prefills + num_decodes;
  // Set once the chunked pipeline has produced the non-spec half of the batch,
  // so the `opt` backend below knows to skip it. Spec-decode batches are never
  // eligible for the chunked path and always fall through.
  bool non_spec_done = false;

#ifdef VLLM_XPU_ENABLE_XE2
  // The chunked pipeline handles non-spec batches only, needs the head
  // dimension to be a whole number of DPAS tiles, and is instantiated for
  // bf16 activations alone (see chunk_kda_xe2.cpp).
  if (non_spec_batch_size > 0 &&
  ...
  if (non_spec_batch_size > 0 && !non_spec_done) {
    const KdaRecurrentBatchArgs args{
        ...
        nullptr,
        non_spec_batch_size};
    // A batch without prefills is one token per sequence, which lets the
    // kernel drop its token loop entirely.
    if (num_prefills == 0) {
