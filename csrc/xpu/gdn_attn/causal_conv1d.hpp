...
// row, with capacity for the configured maximum speculative width. The
// accepted window starts at row num_accepted_tokens-1; after processing, the
// active prefix (shifted history + this request's query tokens) is written
 ...
 public:
  ...
  operator()(sycl::nd_item<2> item) const {
    ...
    for (int t_local = 0; t_local < query_len; ++t_local) {
      const int token_id_local = seq_start + t_local;
...
#pragma unroll
    ...
    const int state_len = Width - 2 + query_len;
...
#pragma unroll
    ...
    // from row init_row+1, rows [hist_rows, state_len) = this request's inputs.
    // In-place left shift is safe since src row (init_row+1+j) >= dst row j.
    if (Width > 1 && has_conv_state) {
      const int hist_rows = state_len - query_len;
...
#pragma unroll
          ...
          const int token_id_local = seq_start + draft_t;
          const int global_t = token_indx[token_id_local];
#pragma unroll
          for (int e = 0; e < elems_per_item; ++e) {
            state_line_ptr[j * conv_elems + reordered_elems_id + e] =
