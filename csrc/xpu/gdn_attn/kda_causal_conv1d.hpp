...
// sequence.
//
// Three kernels cover the shapes the scheduler produces; `launch_kda_conv` in
// kda_attention_interface.cpp picks between them:
//
//   * `causal_conv1d_kernel<Width, IsSpec>` - the general one. One thread per
//     (sequence, channel); it walks the sequence serially, keeping the `Width`
//     tap window in registers. `IsSpec` switches the state cache between one
//     slot per sequence (non-spec) and one slot per accepted token (spec
//     decode). Handles every batch shape.
//   * `causal_conv1d_tiled_kernel<Width, TileT>` - long-prefill specialisation.
//     The general kernel launches only batch*channels threads, which starves
//     the GPU when a few long sequences carry all the tokens. This one splits
//     each sequence into `TileT`-token tiles and recomputes the `Width - 1`
//     token halo per tile, trading a little arithmetic for parallelism.
//   * `causal_conv1d_update_state_kernel<Width>` - the tiled kernel does not
//     touch the cache, so this writes back the trailing `Width - 1` tokens of
//     every sequence afterwards.
