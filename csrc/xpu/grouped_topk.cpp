...
template <typename T, int MAX_EXPERT_GROUPS>
void launch_fused_grouped_topk(sycl::queue& queue, float* topk_weights,
                               int* topk_ids, const T* gating_output,
                               const T* e_score_correction_bias,
                               const ScoringFunc scoring_mode,
                               const bool renormalize, const int tokens,
  ...
  queue.submit([&](sycl::handler& cgh) {
    Kernel task(topk_weights, topk_ids, gating_output, e_score_correction_bias,
                scoring_mode, renormalize, tokens, experts, top_k,
...
#define CASE_TOPK(K)                                                           \
  case K:                                                                      \
    launch_fused_grouped_topk<T, K>(                                           \
        ...
        scoring_mode, renormalize, tokens, experts, top_k, num_expert_group,   \
        topk_group);                                                           \
    break;
