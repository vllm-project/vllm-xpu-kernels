...
VLLM_TORCH_LIBRARY_IMPL_EXPAND(VLLM_TORCH_OP_NAMESPACE, XPU, ops) {
  ...
  VLLM_TORCH_IMPL(ops, "gemma_rms_norm", &gemma_rms_norm);
  VLLM_TORCH_IMPL(
      ops, "fused_add_gemma_rms_norm", &fused_add_gemma_rms_norm);
  VLLM_TORCH_IMPL(ops, "layer_norm", &layer_norm);
  VLLM_TORCH_IMPL(ops, "fused_add_layer_norm", &fused_add_layer_norm);
  VLLM_TORCH_IMPL(ops, "nemotron_layer_norm", &nemotron_layer_norm);
  VLLM_TORCH_IMPL(
      ops, "fused_add_nemotron_layer_norm", &fused_add_nemotron_layer_norm);
  VLLM_TORCH_IMPL(
      ops,
  ...
  VLLM_TORCH_IMPL(ops, "situ_and_mul", &situ_and_mul);
...
VLLM_TORCH_LIBRARY_IMPL_EXPAND(
  ...
  VLLM_TORCH_IMPL(ops, "xpu_host_register", &xpu_host_register);
  VLLM_TORCH_IMPL(ops, "xpu_host_unregister", &xpu_host_unregister);
}
