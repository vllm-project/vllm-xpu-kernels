...
A [vLLM](https://github.com/vllm-project/vllm) plugin that provides optimized custom kernels for Intel GPUs (XPU) to accelerate LLM inference.
...
| **KV Cache** | `reshape_and_cache`, `reshape_and_cache_flash` |
| **Mixture of Experts** | TopK (softmax/sigmoid), grouped TopK, fused grouped TopK, MoE align, MoE gather, expert remapping |
...
### Why statically link oneDNN?

`vllm-xpu-kernels` **statically links oneDNN (DNNL)** rather than depending on a shared library. The reasons are:

- **Version compatibility** — Static linking pins the exact oneDNN version shipped with the package, preventing breakage from incompatible system-installed versions.
- **Performance consistency** — Avoids variability from host system builds or configurations of oneDNN.
- **Reliability** — Eliminates runtime linker failures caused by missing or incorrect `LD_LIBRARY_PATH` entries.
- **Alignment with PyTorch** — PyTorch itself statically links oneDNN for the same reasons, making this approach consistent with the broader ecosystem.
