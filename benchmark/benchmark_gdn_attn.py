...
if __name__ == "__main__":
    ...
    # Validate arguments
    if args.cache_batch_size < args.batch_size:
        raise ValueError(
            "--cache-batch-size should be no smaller than --batch-size")
    if args.num_k_heads % args.tp_size != 0:
        raise ValueError("--num-k-heads should be divisible by --tp-size")
    if args.num_v_heads % args.tp_size != 0:
        raise ValueError("--num-v-heads should be divisible by --tp-size")
    if args.num_v_heads % args.num_k_heads != 0:
        raise ValueError("--num-v-heads should be divisible by --num-k-heads")
