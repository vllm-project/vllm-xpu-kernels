...
def mhc_pre_reference(
    ...
    layer_input = torch.einsum("nj,njh->nh", pre_mix, res_fp32).to(
        torch.bfloat16
    )
...
def test_mhc_pre(num_tokens: int, hidden_size: int, hc_mult: int, test_data):
    ...
    torch.testing.assert_close(
        layer_input, layer_input_ref, atol=1e-2, rtol=1e-2
    )
...
@pytest.mark.parametrize("hidden_size", [1024, 2048])
@pytest.mark.parametrize("hc_mult", [4])
def test_hc_head_fused(
    num_tokens: int, hidden_size: int, hc_mult: int, test_data
):
    device, dtype = test_data
