...
def _build_and_run_align(
    *,
    ...
    topk_ids = topk_ids.to("xpu")
    token_lora_mapping = token_lora_mapping.to("xpu")
    ...
    if lora_ids_override is None:
        lora_ids = torch.full(
            (max_loras + 1,), -1, dtype=torch.int32, device="xpu"
        )
        ...
        lora_ids = lora_ids_override.to(dtype=torch.int32, device="xpu")

    adapter_enabled = torch.ones(
        (max_loras + 1,), dtype=torch.int32, device="xpu"
    )
    ...
    num_tokens_post_pad = torch.full(
        (max_loras,), SENTINEL_NPAD, dtype=torch.int32, device="xpu"
    )
...
def test_moe_lora_align_block_size_lora_id_oob_guard():
    ...
    guard, an illegal-memory-access would surface on the next XPU sync.
    """
    max_loras = 1
