...
def ref_moe_gather(output, moe_output, topk_weights,
                   ...
                   unpermuted_row_to_permuted_row, expert_first_token_offset, num_experts):
    ...
    if expert_first_token_offset[-1] == 0:
        ...
        selected_outputs = selected_outputs.view([input_len, topk, hidden_size])
    else:
        selected_outputs = moe_output[moe_indices]  # (input_len, topk, hidden_size)
        ...
        selected_outputs.view([input_len * topk, hidden_size])[transposed_idx] = moe_output[0]
...
def test_moe_gather(input_len, hidden_size, num_experts, topk, ep_rank, ep_size, dtype):
    seed_everything(7)
    ...
    expert_row_counts = torch.bincount(expert_indices.flatten(), minlength=num_experts)
    expert_row_counts = expert_row_counts[expert_start_id: expert_end_id]
    expert_cumsum = torch.cumsum(expert_row_counts, dim=0)
    expert_first_token_offset = torch.cat([torch.tensor([0], device=DEVICE), expert_cumsum])

    permuted_row_to_unpermuted_row = torch.randperm((input_len * topk),
                                                    dtype=torch.int32,
                                                    device=DEVICE)
    unpermuted_row_to_permuted_row = torch.argsort(permuted_row_to_unpermuted_row).to(torch.int32)
    permuted_row_to_unpermuted_row[expert_cumsum[-1]:] = 0
    unpermuted_row_to_permuted_row[unpermuted_row_to_permuted_row >= expert_cumsum[-1]] = 0

    output = torch.empty((input_len, hidden_size), device=DEVICE, dtype=dtype)
    torch.ops._moe_C.moe_gather(output, moe_output, expert_scores, permuted_row_to_unpermuted_row,
                                unpermuted_row_to_permuted_row, expert_first_token_offset, num_experts_per_node)

    ref_output = torch.empty((input_len, hidden_size),
                             device=DEVICE,
                             dtype=dtype)
    ref_moe_gather(ref_output, moe_output, expert_scores,
                   permuted_row_to_unpermuted_row, unpermuted_row_to_permuted_row, expert_first_token_offset, num_experts_per_node)
