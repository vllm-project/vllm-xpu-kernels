...
def ref_moe_gather(output, moe_output, topk_weights,
    ...
    output_fp32 = torch.einsum('ijk,ij->ik', selected_outputs_fp32, topk_weights_fp32)
...
def test_moe_gather(input_len, hidden_size, num_experts, topk, dtype):
    ...
    expert_scores, _ = torch.topk(scores,
                                               k=topk,
                                               dim=-1,
                                               sorted=False)
    ...
    ref_output = torch.empty((input_len, hidden_size), device=DEVICE, dtype=dtype)
