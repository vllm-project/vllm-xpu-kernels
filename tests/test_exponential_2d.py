...
class ExponentialDistributionTester:
    ...
    def distribution_test_adaptive(samples, lambda_param=1.0, base_alpha=0.05):
        ...
        # KS test
        ks_stat, ks_p_value = stats.kstest(samples_np, 
                                        lambda x: stats.expon.cdf(x, scale=1/lambda_param))
        
        # Anderson-Darling test
        try:
            ad_stat, ad_critical_values, ad_significance_levels = stats.anderson(samples_np, 'expon')
...
def test_exponential_2d_comprehensive(batch_size, vocal_size, seed, offset, lambda_param):
    seeds = torch.tensor([seed, offset], dtype=torch.int64, device=torch.device("cpu"))

    samples = torch.empty(batch_size, vocal_size, dtype=torch.float, device=DEVICE)
    torch.ops._xpu_C.exponential_2d_(samples, seeds, lambda_param)
    
    assert samples.shape == (batch_size, vocal_size), f"Shape mismatch: expected {(batch_size, vocal_size)}, got {samples.shape}"
    assert torch.all(samples > 0), "All samples should be positive for exponential distribution"
    ...
    if total_samples >= 1000:
        ...
        stats_results = tester.basic_statistics_test(samples, lambda_param, tolerance=0.15)

        if total_samples >= 5000:
            assert stats_results['mean_test_passed'], f"Mean test failed: error = {stats_results['mean_error']:.4f}"
            assert stats_results['var_test_passed'], f"Variance test failed: error = {stats_results['var_error']:.4f}"
        
        if total_samples >= 2000:
            dist_results = tester.distribution_test_adaptive(samples, lambda_param)

            if total_samples >= 10000:
                pass
            else:
                assert dist_results['ks_test_passed'], f"KS test failed: p-value = {dist_results['ks_p_value']:.4f}"
        ...
        if total_samples >= 1000:
            ...
            if runs_test['p_value'] is not None:
                if total_samples >= 5000 and runs_test['test_passed'] is not None:
                    # Run tests sometimes fail due to random fluctuations, so we employ more lenient criteria.
                    if not runs_test['test_passed']:
                        warnings.warn(f"Runs test failed: p-value = {runs_test['p_value']:.4f}")
...
@pytest.mark.parametrize("offset", [0, 1, 10])
@pytest.mark.parametrize("lambda_param", [0.5, 1.0, 2.0])
def test_exponential_2d_reproducibility(batch_size, vocal_size, seed, offset, lambda_param):
    seeds1 = torch.tensor([seed, offset], dtype=torch.int64, device=torch.device("cpu"))
    samples1 = torch.empty(batch_size, vocal_size, dtype=torch.float, device=DEVICE)
    torch.ops._xpu_C.exponential_2d_(samples1, seeds1, lambda_param)
    
    seeds2 = torch.tensor([seed, offset], dtype=torch.int64, device=torch.device("cpu"))
    samples2 = torch.empty(batch_size, vocal_size, dtype=torch.float, device=DEVICE)
    torch.ops._xpu_C.exponential_2d_(samples2, seeds2, lambda_param)

    assert torch.allclose(samples1, samples2), "Same seeds should produce identical results"
    
    seeds3 = torch.tensor([seed + 10, offset + 10], dtype=torch.int64, device=torch.device("cpu"))
    samples3 = torch.empty(batch_size, vocal_size, dtype=torch.float, device=DEVICE)
    torch.ops._xpu_C.exponential_2d_(samples3, seeds3, lambda_param)
    
    assert not torch.allclose(samples1, samples3), "Different seeds should produce different results"
...
def test_exponential_2d_edge_cases(batch_size, vocal_size, seed, offset, lambda_param):
    seeds = torch.tensor([seed, offset], dtype=torch.int64, device=torch.device("cpu"))
    ...
    assert torch.all(samples > 0), f"All samples should be positive for λ={lambda_param}"
    assert torch.all(torch.isfinite(samples)), f"All samples should be finite for λ={lambda_param}"

    sample_mean = torch.mean(samples).item()
    theoretical_mean = 1.0 / lambda_param
    relative_error = abs(sample_mean - theoretical_mean) / theoretical_mean

    assert relative_error < 0.5, f"Mean too far from theoretical for λ={lambda_param}: {sample_mean:.4f} vs {theoretical_mean:.4f}"
