...
from tests.nn.utils import (DTYPE_BITS_MAPPING, VLLM_ACT_QUANT_MODE_TO_XPU,
                            XPU_UNSUPPORTED_ACT_QUANT_MODES, QuantDtype,
                            QuantMethod, XPUWoqActQuantMode)
...
def pack(imatrix: torch.Tensor, direction: str = "column"):
    ...
    if direction == "column":
        ...
        qmatrix = torch.bitwise_left_shift(imatrix,
                                           shifts[None, None, :]).sum(dim=-1)

    elif direction == "row":
        imatrix = imatrix.view(imatrix.shape[0] // (32 // 4), (32 // 4), -1)
        qmatrix = torch.bitwise_left_shift(imatrix, shifts[None, :,
                                                           None]).sum(dim=1)
...
def fast_awq_to_gptq(qweight, qzeros):
    ...
    izeros = apply_order(izeros,
                         direction="column",
                         order=REVERSE_AWQ_PACK_ORDER)
    iweights = apply_order(iweights,
                           direction="column",
                           order=REVERSE_AWQ_PACK_ORDER)
...
class WeightOnlyQuantizedLinear(nn.Module):
    ...
    def __init__(
        ...
        self.device = device
        # `compression_dim`: dimension to be compressed in data.
        ...
        # `use_optimum_format` is for GPTQ model,
        #   if it is True, it's weight is k x n,
    ...
    def transpose_onednn_woq_format(self):
        ...
        # - Weights need to be contiguous along the 'k' dimension, but the shape
        #   should remain (k, n/8).
    ...
    def from_weight(
        ...
        **kwargs,
        ...
        Args:
            qweight (Tensor): in int32 dtype and contains actually int4 data
            ...
            qconfig (object): quantization recipe for Weight only quantization.
                Default value is ``None``.
            bias (Tensor or None): bias for linear
            group_size: Group size for weight quantization
            g_idx: Indices of groups for each input channel of weight.
                 Generated by GPTQ with act-order.
        ...
        ), "Only support INT4 FP8_E5M2 FP8_E4M3FN as quantization data type."
        assert quant_method in [
            QuantMethod.GPTQ_GEMM,
            QuantMethod.AWQ_GEMM,
        ], "Only support GPTQ_GEMM and AWQ_GEMM as quantization method."
        ...
        if (qconfig is not None and hasattr(qconfig, "global_qconfig")
                and hasattr(qconfig.global_qconfig, "act_quant_mode")):
        ...
        cls_inst = WeightOnlyQuantizedLinear(
            ...
            compress_statistics=True,
            weight_dtype=(
                "int4_fullrange" if dtype == QuantDtype.INT4 else
                "fp8_e5m2" if dtype == QuantDtype.FP8_E5M2 else "fp8_e4m3fn"),
        ...
        if quant_method == QuantMethod.AWQ_GEMM:
            qweight_new, qzeros_new = fast_awq_to_gptq(cls_inst.qweight,
                                                       cls_inst.qzeros)
            if qweight_new.shape != cls_inst.qweight.data.shape:
                cls_inst.qweight.data = cls_inst.qweight.data.view_as(
                    qweight_new)
        ...
        if ((dtype == QuantDtype.FP8_E5M2 or dtype == QuantDtype.FP8_E4M3FN)
                and qweight.shape[0] != in_feature):
            cls_inst.weight_transposed = True
