...
def test_per_block_fp8_quant(
    ...
    x = (torch.rand(num_tokens_block_quant, hidden_size_block_quant, dtype=dtype, device="xpu") + 1e-6
         )  # avoid nans
    ...
    assert torch.allclose(ref_out.float(), ops_out.float(), atol=0.15, rtol=0.15)
    assert torch.allclose(ref_scales.float(), ops_scales.float(), atol=0.01, rtol=0.01)


MXFP8_DTYPES = [torch.float, torch.bfloat16]
@pytest.mark.parametrize("num_tokens_block_quant", NUM_TOKENS_BLOCK_QUANT)
@pytest.mark.parametrize("hidden_size_block_quant", HIDDEN_SIZES_BLOCK_QUANT)
@pytest.mark.parametrize("mxfp8_dtypes", MXFP8_DTYPES)
@pytest.mark.parametrize("seed", SEEDS)
@torch.inference_mode()
def test_per_block_mxfp8_quant(
    num_tokens_block_quant: int,
    hidden_size_block_quant: int,
    mxfp8_dtypes: torch.dtype,
    seed: int,
) -> None:
    seed_everything(seed)

    x = (torch.rand(num_tokens_block_quant, hidden_size_block_quant, dtype=mxfp8_dtypes, device="xpu") + 1e-6
         )  # avoid nans
