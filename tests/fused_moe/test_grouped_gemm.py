...
def test_grouped_gemm(m, n, k, e, topk, dtype, has_bias):
    ...
    cutlass_grouped_gemm(input_A, input_B, None, bias, output,
...
def test_grouped_gemm_fp8(m, n, k, e, topk, dtype, fp8_dtype, has_bias):
    seed_everything(7)
    num_experts = e
    total_m = m * topk
    # input
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
@pytest.mark.parametrize("has_bias", [False, True])
def test_grouped_gemm_int4(m, n, k, e, topk, dtype, has_bias):
    seed_everything(7)
    num_experts = e
    group_size = 128
    ...
    output = torch.empty((total_m, n), dtype=dtype, device=DEVICE)
    cutlass_grouped_gemm(input_A, input_B_int4, scale_B, bias, output,
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
@pytest.mark.parametrize("has_bias", [False, True])
def test_grouped_gemm_mxfp4(m, n, k, e, topk, dtype, has_bias):
    seed_everything(7)
    num_experts = e
    group_size = 32
    group_num = k // group_size
