...
"""Cross-check the KDA chunked prefill backend against the recurrent one.
...
CASES = {
    ...
    # notice and hand the batch to `opt` rather than return the diverged
    # result.
...
def test_chunk_matches_the_recurrent_backend(tmp_path):
    """The chunked pipeline has to reproduce the sequential recurrence.

    `opt` walks the sequence one token at a time, so it is the operational
    definition of the recurrence; `tests/gdn_attn/test_kda_attention.py` pins
    it against a torch reference. Here it is the golden the chunked pipeline
    has to match across head dims, state dtypes, gate modes, permuted token
    maps and mixed decode+prefill batches -- including the `_lbsat` cases,
    where matching means noticing the clamp and falling back rather than
    returning a diverged result.
    """
    reference = _collect("opt", tmp_path)
    actual = _collect("chunk", tmp_path)
    ...
    for name in reference:
        ...
        torch.testing.assert_close(
            ...
            msg=lambda m, n=name: f"chunk/{n} output mismatch\n{m}")
        torch.testing.assert_close(
            act_state, ref_state, atol=6e-2, rtol=6e-2,
            msg=lambda m, n=name: f"chunk/{n} state mismatch\n{m}")


@pytest.mark.skipif(not torch.xpu.is_available(), reason="requires XPU")
def test_removed_backend_is_rejected(tmp_path):
    """`opt` superseded the unvectorised reference kernel, which was deleted.

    Accepting its name again would silently reintroduce a third backend, so the
    op has to reject it rather than fall back.
    """
    env = dict(os.environ)
    env["VLLM_XPU_KDA_RECURRENT_MODE"] = "recurrent"
    env.setdefault("PYTHONPATH", os.getcwd())
    done = subprocess.run(
        [sys.executable, os.path.abspath(__file__),
         str(tmp_path / "rejected.pt")],
        env=env, capture_output=True, text=True)
    assert done.returncode != 0, "the removed backend name was accepted"
    assert "auto/opt/chunk" in done.stderr
...
def test_chunk_strict_rejects_saturating_decay(tmp_path):
    ...
    default configuration falls back to `opt`, which the `_lbsat` cases above
    cover; the strict switch turns the same condition into an error naming that
    fallback instead."""
